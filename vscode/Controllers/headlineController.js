...
export const createHeadline = async (req, res, next) => {
    try {
        ...
        let countryId = null;
        if (country) {
            const c = await Country.findOne({ name: country });
            if (c) countryId = c._id;
        }
        ...
        let districtId = null;
        if (district) {
            const d = await City.findOne({ name: district });
            if (d) districtId = d._id;
        }
        ...
        const populatedHeadline = await savedHeadline.populate([
...
// Get All Headlines ======================================================================================================================
// Get All Headlines ======================================================================================================================
// यह फंक्शन सभी हेडलाइंस को लाता है, भले ही उनमें कुछ फ़ील्ड्स मौजूद न हों।
export const getAllHeadlines = async (req, res, next) => {
    try {
        // find({}) का मतलब है कि कलेक्शन से सभी दस्तावेज़ प्राप्त करें।
        // यह उन दस्तावेज़ों को फ़िल्टर नहीं करेगा जिनमें newsId, category, या subCategory नहीं है।
        const headlines = await headlineModel.find({})
            // populate उन फ़ील्ड्स पर काम करेगा जिनमें ObjectId है।
            // अगर newsId जैसा कोई फ़ील्ड null है, तो populate उसे अनदेखा कर देगा और वह null ही रहेगा।
            ...
            .populate('newsId', 'slug_en ') // अगर newsId मौजूद नहीं है, तो यह हेडलाइन को नहीं हटाएगा।
            .populate('state', 'name iso2')
            .populate('district', 'name')
            .sort({ createdAt: -1 })
            .limit(5); 

        // यह मैपिंग हर हेडलाइन के लिए category और subCategory की वैल्यू को कॉपी करता है।
        // अगर ऑरिजनल वैल्यू null है, तो नई वैल्यू भी null होगी।
        const finalHeadlines = headlines.map(headline => {
            const h = headline.toObject();
            h.category_name = headline.category; // अगर category नहीं है, तो यह null होगा।
            h.subCategory_name = headline.subCategory; // अगर subCategory नहीं है, तो यह null होगा।
            return h;
        });
