...
// //         if (res?.success) {
// //           // ✅ Filtering ONLY "Indepth Express" category
// //           const indepthNews = res?.data?.filter((item) => {
// //             const categoryName = item?.category?.name?.toLowerCase() || "";
// //             return categoryName === "indepth express";
// //           });
// //           setNewsData(indepthNews || []);
// //         } else {
// //           setError(res?.message || "Failed to load Indepth Express news.");
// //         }
// //       } catch (err) {
...
// //         <p className="mt-2">Indepth Express लोड हो रहे हैं...</p>
// //       </div>
// //     );
// //   }
...
// //               className="me-2"
// //             ></div>
// //             <h5 className="fw-bold m-0">Indepth Express</h5>
// //           </div>
// //           <hr
...
// //           कोई Indepth Express समाचार उपलब्ध नहीं है।
// //         </Alert>
// //       </Container>
// //     );
// //   }
...
// //                 alt={mainArticle?.title_hi || mainArticle?.title_en || "Indepth Express"}
// //                 width="100%"
// //                 height="320px"
// //                 objectFit="cover"
// //                 borderRadius="8px"
...
// //                     alt={bottomArticle?.title_hi || bottomArticle?.title_en || "Indepth Express"}
// //                     width="100%"
// //                     height="90px"
// //                     objectFit="cover"
// //                     borderRadius="8px"
...
// //                     <MediaRenderer
// //                       media={article?.media}
// //                       alt={article?.title_hi || article?.title_en || "Indepth Express"}
// //                       width="100%"
// //                       height="90px"
// //                       objectFit="cover"
// //                       borderRadius="8px"
...
// // export default IndepthExpressSection;
...
//     const commonStyles = { 
//     width: "100%", 
//     // 770/500 का मतलब है 1.54 का रेशियो। 
//     // इससे इमेज कभी नहीं कटेगी और उसी शेप में रहेगी।
//     aspectRatio: "770 / 500", 
//     objectFit: "cover", 
//     borderRadius, 
//     backgroundColor: "#e0e0e0", 
...
//       alt={alt}
//       style={commonStyles}
//       onError={(e) => { e.target.src = `https://via.placeholder.com/${parseInt(width)}x${parseInt(height)}?text=Error`; }}
//     />
//   );
// };
...
//           // ✅ Filtering ONLY "Indepth Express" category
//           const indepthNews = res?.data?.filter((item) => 
//             item?.category?.name?.toLowerCase() === "indepth express"
//           );
//           setNewsData(indepthNews || []);
//         } else {
...
//       {/* ✅ Content */}
//       {newsData.length === 0 ? (
//         <Alert variant="info">कोई Indepth Express समाचार उपलब्ध नहीं है।</Alert>
//       ) : (
//         <Row>
//           {/* Left Column */}
//           <Col lg={7} className="mb-4 mb-lg-0 d-flex flex-column">
//             {mainArticle && (
//               <Link to={`/news/${mainArticle.slug_en || mainArticle._id}`} state={{ relatedArticles: newsData }} style={linkStyle} className="d-block position-relative mb-4 flex-grow-1">
//                 <MediaRenderer media={mainArticle.media} alt={mainArticle.title_hi} width="100%" height="320px" />
//                 <div className="position-absolute bottom-0 start-0 text-white w-100 p-3" style={{ background: "linear-gradient(to top, rgba(0,0,0,0.85), transparent)", borderRadius: "0 0 8px 8px" }}>
//                   {/* <h5 className="fw-bold">{mainArticle.title_hi || mainArticle.title_en}</h5> */}
//                                <div 
...
//                     <small className="ms-2 text-light">{mainArticle.createdBy?.name || "EMS"} | {formatFullDateTime(mainArticle.publishedAt)}</small>
//                   </div>
//                 </div>
//               </Link>
//             )}
//             {bottomArticle && (
//               <Link to={`/news/${bottomArticle.slug_en || bottomArticle._id}`} state={{ relatedArticles: newsData }} style={linkStyle}>
//                 <Row className="align-items-center">
//                   <Col xs={4} sm={3}><MediaRenderer media={bottomArticle.media} alt={bottomArticle.title_hi} width="100%" height="90px" /></Col>
//                   <Col xs={8} sm={9} className="ps-2">
...
//                       <small className="text-muted ms-1">{bottomArticle.createdBy?.name || "EMS"} | {formatFullDateTime(bottomArticle.publishedAt)}</small>
//                     </div>
//                   </Col>
//                 </Row>
//               </Link>
...
//                   <Row className="align-items-center">
//                     <Col xs={4}><MediaRenderer media={article.media} alt={article.title_hi} width="100%" height="90px" /></Col>
//                     <Col xs={8} className="ps-2">
//                       {/* <p className="fw-bold mb-1" style={{ fontSize: "0.9rem", lineHeight: "1.4" }}>{article.title_hi || article.title_en}</p> */}
...
//                         <small className="text-muted ms-1">{article.createdBy?.name || "EMS"} | {formatFullDateTime(article.publishedAt)}</small>
//                       </div>
//                     </Col>
//                   </Row>
//                 </Link>
...
const IndepthExpressSection = () => {
  ...
  const newsData = categorized["indepth express"] || categorized["इंडेप्थ एक्सप्रेस"] || [];
                ...
                >
                  ...
                  <div className="d-flex align-items-center mt-2">
                    ...
                    <small className="ms-2 text-light">
                      {mainArticle.createdBy?.name || "EMS"} | {formatFullDateTime(mainArticle.publishedAt)}
                    </small>
                  </div>
                </div>
              </Link>
                  ...
                  <Col xs={8} sm={9} className="ps-2">
                    ...
                    <div className="d-flex align-items-center mt-1">
                      ...
                      <small className="text-muted ms-1">
                        {bottomArticle.createdBy?.name || "EMS"} | {formatFullDateTime(bottomArticle.publishedAt)}
                      </small>
                    </div>
                  </Col>
                </Row>
                  ...
                  <Row className="align-items-center">
                    ...
                    <Col xs={8} className="ps-2">
                      ...
                      <div className="d-flex align-items-center mt-1">
                        <UserAvatar user={article.createdBy} size={20} />
                        <small className="text-muted ms-1">
                          {article.createdBy?.name || "EMS"} | {formatFullDateTime(article.publishedAt)}
                        </small>
                      </div>
                    </Col>
                  </Row>
