...
//         const tags = [];
//         if (article.category?.name && !["business", "बिज़नेस"].includes(article.category.name.toLowerCase()))
...
//         const summaryText = getTwoLinesCleanText(
...
const BusinessListPage = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        // 1. Process News (Filter for Business)
        if (newsRes?.success) {
          const businessNews = newsRes.data.filter((item) => {
            const categoryName =
        ...
        // 2. Process Ads (Filter for Business)
        if (adsRes?.success && Array.isArray(adsRes.ads)) {
          
          const isBusinessAd = (ad) => {
            return (
              ad.categories &&
              ad.categories.some(
                (cat) =>
                  cat.toLowerCase() === "business" ||
                  cat.toLowerCase() === "बिज़नेस"
              )
            );
          };

          const inlineAd = adsRes.ads.find(
            (a) => a.position === "inline" && isBusinessAd(a)
          );
          
          const inlineLargeAd = adsRes.ads.find(
            (a) => a.position === "inlineLarge" && isBusinessAd(a)
          );
          
          const inline2Ad = adsRes.ads.find(
            (a) => a.position === "inline2" && isBusinessAd(a)
          );
          
          const bottomAd = adsRes.ads.find(
            (a) => a.position === "bottom" && isBusinessAd(a)
          );
  ...
  // --- Render News Card Helper (Kept your custom styling & Tags logic) ---
  const renderNewsCard = (article) => {
    const tags = [];
    if (
      article.category?.name &&
      !["business", "बिज़नेस"].includes(article.category.name.toLowerCase())
    ) {
      tags.push(article.category.name);
    }
    if (article.subCategory?.name) tags.push(article.subCategory.name);
