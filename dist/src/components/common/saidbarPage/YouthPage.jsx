...
// // // Clean summary (same as sports)
// // const getTwoLinesCleanText = (text) => {
// //   if (!text) return "";
// //   let cleanText = text.replace(/<[^>]*>/g, "").replace(/\s+/g, " ").trim();
// //   cleanText = cleanText.replace(
// //     /Your browser does not support the video tag\./g,
// //     ""
// //   );
...
// //       <h2 className="fw-bold border-bottom pb-2 mb-4">लाइफस्टाइल</h2>
...
//         // 1. Process News (Filtered by "Youth")
//         if (newsRes?.success) {
//           const youthNews = newsRes.data.filter((item) => {
//             const categoryName =
//               typeof item.category === "string"
//                 ? item.category.toLowerCase()
//                 : item.category?.name?.toLowerCase() || "";
//             // Logic updated to STRICTLY Youth as requested
...
//         // 2. Process Ads (Filtered by "Youth")
...
//               ad.categories.some(
//                 (cat) =>
//                   cat.toLowerCase() === "youth" ||
//                   cat.toLowerCase() === "युवा"
//               )
//             );
//           };

//           const inlineAd = adsRes.ads.find(
//             (a) => a.position === "inline" && isYouthAd(a)
//           );
          
//           const inlineLargeAd = adsRes.ads.find(
//             (a) => a.position === "inlineLarge" && isYouthAd(a)
//           );
          
//           const inline2Ad = adsRes.ads.find(
//             (a) => a.position === "inline2" && isYouthAd(a)
//           );
          
//           const bottomAd = adsRes.ads.find(
//             (a) => a.position === "bottom" && isYouthAd(a)
//           );
...
//   // Helper to render a single news card (kept exactly as per your CSS)
...
//                   alt={article.title_hi || article.title}
//                   onError={(e) =>
//                     (e.target.src = "https://via.placeholder.com/150x90?text=Error")
//                   }
//                 />
//               )}
//             </Col>
...
//       {/* Updated Title to Youth/युवा based on logic */}
//       <h2 className="fw-bold border-bottom pb-2 mb-4">युवा (Youth)</h2>
...
const YouthListPage = () => {
          ...
          const lifestyleNews = newsRes.data.filter((item) => {
            const categoryName =
              typeof item.category === "string"
                ? item.category.toLowerCase()
                : item.category?.name?.toLowerCase() || "";
            // 🔴 Category updated to Lifestyle as requested
            return categoryName === "lifestyle" || categoryName === "लाइफस्टाइल";
          });
  ...
  return (
    <Container className="my-4">
      ...
      <h2 className="fw-bold border-bottom pb-2 mb-4">Lifestyle</h2>
      ...
      {/* 2. First 5 News */}
      {newsData.length === 0 && !loading && (
        <Alert variant="warning">कोई Lifestyle खबर उपलब्ध नहीं है।</Alert>
      )}
      {newsData.slice(0, 5).map((article) => renderNewsCard(article))}
